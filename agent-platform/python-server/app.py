...
def main():
    ...
    if llm_provider == "groq":
        params = llms[0].get("parameters", {})
        groq_url = params.get("endpoint", "https://api.groq.com/openai/v1/chat/completions")
        groq_api_key = secrets.get(api_key, os.environ.get(api_key, ""))
        prompt = nodes[0].get("prompt", "Hello from Groq!") if nodes else "Hello from Groq!"
        messages = [{"role": "user", "content": prompt}]
        payload = {
            "messages": messages,
            "model": llm_model,
            "temperature": float(params.get("temperature", 1)),
            "max_completion_tokens": int(params.get("max_completion_tokens", 8192)),
            "top_p": float(params.get("top_p", 1)),
            "stream": params.get("stream", "true") == "true",
